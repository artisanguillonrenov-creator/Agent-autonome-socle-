...
const secretContent=/(-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----|\b(?:api[_-]?key|access[_-]?token|client[_-]?secret|token|password|github_token|openrouter_api_key|api_token)\s*[:=]\s*["']?[A-Za-z0-9_\-]{12,})/i;
...
export class GitHubRepositoryReader {
  ...
  async searchCodePaths(target:RepositoryTarget,terms:string[],maxResults=this.limits.maxSearchResults):Promise<RepositoryTreeEntry[]>{if(!this.octokit.rest.search?.code)return[];const found=new Map<string,RepositoryTreeEntry>();for(const term of terms){if(found.size>=maxResults)break;try{const r=await this.octokit.rest.search.code({q:`${term} repo:${target.owner}/${target.repo}`,per_page:Math.min(maxResults,100)});for(const item of r.data.items??[]){if(found.size>=maxResults)break;if(typeof item.path==="string"&&!secretName.test(item.path)&&this.isTextPath(item.path))found.set(item.path,{path:item.path,type:"blob",sha:item.sha});}}catch{continue;}}return[...found.values()];}
  async readPullRequest(target:RepositoryTarget,pullNumber:number){try{const [pr,files]=await Promise.all([this.octokit.rest.pulls.get({owner:target.owner,repo:target.repo,pull_number:pullNumber}),this.octokit.paginate(this.octokit.rest.pulls.listFiles,{owner:target.owner,repo:target.repo,pull_number:pullNumber,per_page:100})]);return{number:pr.data.number,title:pr.data.title,state:pr.data.state,base:pr.data.base.ref,head:pr.data.head.ref,...this.safeDiffFiles(files)};}catch(e){this.translate(e);}}
  async readCommit(target:RepositoryTarget,ref:string){try{const r=await this.octokit.rest.repos.getCommit({owner:target.owner,repo:target.repo,ref});return{sha:r.data.sha,message:r.data.commit.message,...this.safeDiffFiles(r.data.files??[])};}catch(e){this.translate(e,"REPOSITORY_REF_NOT_FOUND");}}
  async readDiff(target:RepositoryTarget,base:string,head:string){try{const r=await this.octokit.rest.repos.compareCommits({owner:target.owner,repo:target.repo,base,head});return{status:r.data.status,...this.safeDiffFiles(r.data.files??[])};}catch(e){this.translate(e,"REPOSITORY_REF_NOT_FOUND");}}
  ...
  private safeDiffFiles(files:any[]){const totalFiles=files.length,allowed=files.filter(x=>typeof x.filename==="string"&&!secretName.test(x.filename)),limited=allowed.slice(0,this.limits.maxSearchResults),safe=limited.map(x=>{const unsafe=typeof x.patch==="string"&&secretContent.test(x.patch);return{path:x.filename,status:x.status,patch:unsafe?undefined:x.patch?.slice(0,4000),redacted:unsafe,patchTruncated:(x.patch?.length??0)>4000};});return{files:safe,truncated:allowed.length>limited.length,totalFiles,returnedFiles:safe.length};}
