...
export function createRuntimeSkills(orchestrator:ServiceOrchestrator,planner:Planner,planRunner:PlanRunner,workflows:WorkflowRegistry,repositoryClient:GithubReadOnlyClient=createGithubReadOnlyClient(),vectorMemory:VectorMemory=new VectorMemory(new LocalHashingEmbeddingProvider())):SkillDefinition[]{
  ...
  define("software_development",schema({objective:{type:"string"},filePath:{type:"string"},instructions:{type:"string"},exactContent:{type:"string"},oldString:{type:"string",description:"Édition ciblée (str-replace) : extrait exact et unique à remplacer dans le fichier existant, préférable à exactContent/instructions pour un correctif localisé (ex. corriger un test qui échoue) — doit être fourni avec newString."},newString:{type:"string",description:"Texte de remplacement pour oldString (peut être vide pour une suppression)."},revertPrNumber:{type:"integer",description:"Annule proprement une PR mono-fichier déjà ouverte par la Software Factory, au lieu d'empiler un nouveau chantier par-dessus un précédent raté : restaure le fichier à son contenu d'avant cette PR. filePath doit être le fichier réellement modifié par revertPrNumber (vérifié, REVERT_FILE_MISMATCH sinon) — l'identifier d'abord via knowledge_search si besoin."},targetBranch:{type:"string"},targetPr:{type:"integer"},createIfMissing:{type:"boolean"},deleteFile:{type:"boolean",description:"Supprime filePath au lieu de le modifier — capacité destructive, opt-in explicite uniquement. Incompatible avec exactContent/oldString+newString/revertPrNumber/createIfMissing. filePath doit déjà exister (DELETE_FILE_NOT_FOUND sinon)."},branchFromSha:{type:"string",description:"Crée la branche depuis ce SHA précis plutôt que le HEAD courant de la branche par défaut (ex. reproduire un état antérieur exact) — doit exister dans le dépôt (BRANCH_FROM_SHA_NOT_FOUND sinon). Incompatible avec targetBranch/targetPr."},constraints:{type:"array"},executionMode:{type:"string",enum:["foreground","background"]},expectedBaseSha:{type:"string",description:"base_sha attendu (HEAD réel de la branche par défaut) — protection STALE_BASE si fourni."},expectedFilePath:{type:"string",description:"Chemin de fichier autorisé par la mission — DIFF_FIDELITY_FAILED si différent de filePath."},expectedChangeType:{type:"string",enum:["create","update"],description:"Intention déclarée : 'create' exige que le fichier n'existe pas encore, 'update' qu'il existe déjà."},allowFullRewrite:{type:"boolean",description:"Autorise explicitement une réécriture qui ne conserverait presque aucune ligne d'origine."}},["objective","filePath"]),async(i,ctx:SkillContext)=>{
    ...
    return JSON.stringify(await dispatch("software_development",i,{filePath:i.filePath,instructions:directives,exactContent:i.exactContent,oldString:i.oldString,newString:i.newString,revertPrNumber:i.revertPrNumber,createIfMissing:i.createIfMissing===true,deleteFile:i.deleteFile,branchFromSha:i.branchFromSha,neverAutoMerge:true,expectedBaseSha:i.expectedBaseSha,expectedFilePath:i.expectedFilePath,expectedChangeType:i.expectedChangeType,allowFullRewrite:i.allowFullRewrite},undefined,ctx.traceId));
  });
