...
export function createRuntimeSkills(orchestrator:ServiceOrchestrator,planner:Planner,planRunner:PlanRunner,workflows:WorkflowRegistry,repositoryReaderFactory:()=>GitHubRepositoryReader=()=>new GitHubRepositoryReader()):SkillDefinition[]{
  ...
  define("software_development",schema({objective:{type:"string"},repository:{type:"string"},filePath:{type:"string"},instructions:{type:"string"},exactContent:{type:"string"},targetBranch:{type:"string"},targetPr:{type:"integer"},constraints:{type:"array",items:{type:"string"}},executionMode:{type:"string",enum:["foreground","background"]}},["objective"]),async i=>{
    ...
    // repository ABSENT keeps the self-referential fallback below; repository PROVIDED but invalid or blank
    // (including an explicit empty string) must fail closed with REPOSITORY_INVALID rather than silently
    // falling back to the self repository.
    if(typeof i.repository==="string"){const trimmed=i.repository.trim();if(!trimmed)throw new Error("REPOSITORY_INVALID: Le dépôt fourni est vide.");const parsed=parseGitHubRepository(trimmed);resolvedRepository=`${parsed.owner}/${parsed.repo}`;}else if(self){const parsed=resolveSelfRepository(undefined,true);resolvedRepository=`${parsed.owner}/${parsed.repo}`;}
