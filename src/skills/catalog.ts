...
const available = new Set(["inspect_task","cancel_task","schedule_task","monitor_condition","web_search","deep_research","knowledge_search","file_management","software_development","execute_workflow","manage_skill","checkpoint_task","restore_task","document_work","spreadsheet_work","data_analysis","database_query","report_generation"]);
...
const descriptions: Partial<Record<CanonicalSkillId,string>> = {
  ...
  web_search:"Recherche des informations actuelles sur le Web.", deep_research:"Effectue une recherche approfondie et documentée.", knowledge_search:"Inspecte un dépôt GitHub en lecture seule : arborescence, fichiers, recherche par chemin/contenu, Pull Requests, commits et diffs, statut CI d'un commit (action CI_STATUS) et protection de branche (action MAIN_PROTECTION), de façon bornée avec troncature signalée et secrets masqués. Strictement lecture seule ; aucune écriture GitHub (réservée à la Software Factory). Si l'utilisateur demande une modification du logiciel Jarvis et que le fichier exact n'est pas encore connu, cette inspection est une étape intermédiaire : une fois le filePath pertinent identifié, poursuivre avec software_development au lieu de s'arrêter sur une simple analyse.", file_management:"Liste, lit, écrit ou supprime un fichier dans un workspace sécurisé.", software_development:"Délègue une modification logicielle à la Software Factory. Le filePath exact est obligatoire. Pour créer volontairement un fichier absent, fournir createIfMissing=true ; sinon un fichier absent doit provoquer un échec récupérable avant toute écriture. Après knowledge_search, poursuivre avec objective, filePath et instructions. Si le résultat est FAILED avec replannable=true et sideEffectState=none, observer à nouveau puis retenter avec des paramètres corrigés ; ne jamais rejouer automatiquement un échec non replannable, partial ou uncertain. Respecter WAITING_PERMISSION/WAITING_INPUT et ne jamais fusionner automatiquement.", execute_workflow:"Exécute un workflow réutilisable actif.", manage_skill:"Administre les préférences de skills et le cycle de vie des workflows.", compare_sources:"Compare plusieurs sources sur un sujet.", monitor_web:"Surveille périodiquement une information Web.",
