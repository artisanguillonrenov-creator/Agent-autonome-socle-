  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === "GET" && pathname === "/api/services") {
        sendJson(res, 200, agent.serviceOrchestrator.registry.listServices().map(s=>({...s,auth:undefined,authType:s.auth.type,authConfigured:s.auth.type==="none"||Boolean(process.env[s.auth.envVar])})));
        return;
      }
      ...
      const workspaceFiles=pathname.match(/^\/api\/workspaces\/([^/]+)\/files$/);if(workspaceFiles){const id=decodeURIComponent(workspaceFiles[1]);if(!workspaceStore.get(id)){sendJson(res,404,{error:"WORKSPACE_NOT_FOUND"});return;}try{if(req.method==="GET"){sendJson(res,200,workspaceStore.listFiles(id));return;}if(req.method==="POST"){const b=JSON.parse((await readBody(req))||"{}");if(typeof b.path!=="string"||(!b.contentBase64&&!Object.hasOwn(b,"text"))||b.contentBase64!==undefined&&(typeof b.contentBase64!=="string"||!/^[A-Za-z0-9+/]*={0,2}$/.test(b.contentBase64)))throw new Error("INVALID_UPLOAD");const data=b.contentBase64!==undefined?Buffer.from(b.contentBase64,"base64"):String(b.text);const file=workspaceStore.writeFile(id,b.path,data);const artifact=artifactStore.createFileArtifact({workspaceId:id,name:file.path,mimeType:typeof b.mimeType==="string"?b.mimeType:undefined,relativePath:file.path,content:Buffer.isBuffer(data)?data:Buffer.from(data)});sendJson(res,201,{file,artifact});return;}if(req.method==="DELETE"){const path=parsedUrl.searchParams.get("path");if(!path)throw new Error("PATH_REQUIRED");workspaceStore.deleteFile(id,path);sendJson(res,200,{ok:true});return;}}catch(e){sendJson(res,400,{error:(e as Error).message});return;}}
