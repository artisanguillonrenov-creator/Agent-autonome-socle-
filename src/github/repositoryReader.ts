...
/** Recognized secret-bearing key names, matched case-insensitively (KEY=value, KEY: value, "KEY": "value", 'KEY': 'value'). */
const SECRET_KEY_NAMES = "TOKEN|PASSWORD|GITHUB_TOKEN|OPENROUTER_API_KEY|API_TOKEN|API_KEY|ACCESS_TOKEN|REFRESH_TOKEN|CLIENT_SECRET|CLIENTSECRET|AUTHORIZATION";
const assignment = new RegExp(`(["']?\\b(?:${SECRET_KEY_NAMES})\\b["']?\\s*[:=]\\s*)("(?:[^"\\\\]|\\\\.)*"|'(?:[^'\\\\]|\\\\.)*'|[^\\s,;)}\\]"']+)`, "gi");
...
export class GitHubRepositoryReader {
  ...
  async searchContent(repository: string, query: string, ref?: string) {
    ...
    // Truthful truncation: any bound that actually prevented inspection of relevant candidates, or more results than were returned.
    const truncated = this.limitState.tree || this.limitState.files || this.limitState.bytes || found.size > matches.length;
    return { matches, recommendedFiles: matches, truncated, filesRead: this.filesRead, bytesRead: this.bytesRead };
  }
  ...
  async context(repository: string, ref?: string): Promise<RepositoryContext> { const tree = await this.tree(repository, ref); return { repository: tree.repository, defaultBranch: tree.defaultBranch, ref: tree.ref, treeEntries: tree.entries.length, filesRead: this.filesRead, bytesRead: this.bytesRead, truncated: tree.truncated }; }
  ...
  async audit(repository: string, ref?: string) {
    ...
    for (const entry of candidates) { if (this.filesRead >= REPOSITORY_LIMITS.maxFilesRead) { this.limitState.files = true; break; } try { const file = await this.readFile(repository, entry.path, tree.ref); inspectedFiles.push(entry.path); contents.set(entry.path, file.content); categories.add(classify(entry.path)); } catch { /* Coverage reports inaccessible files rather than fabricating findings; limitState already recorded why by readFile. */ } }
    const findings: AuditFinding[] = [];
    for (const [path, content] of contents) { const dangerous = content.split(/\r?\n/).find(line => /(?:rejectUnauthorized\s*[:=]\s*false|NODE_TLS_REJECT_UNAUTHORIZED\s*=\s*["']?0|(?:disable|skip|bypass)[_-]?auth\s*[:=]\s*true|auth\s*[:=]\s*false)/i.test(line)); if (dangerous) findings.push({ severity: "HIGH", title: "Dangerous security configuration is enabled", files: [path], evidence: dangerous.trim().slice(0, 240), impact: "Authentication or TLS verification can be bypassed in this configuration.", recommendation: "Remove the bypass and require secure authentication/TLS verification in every environment." }); }
    ...
    const inspectionSufficient = !limitsReached.tree && !limitsReached.files && !limitsReached.bytes && inspectedFiles.length >= Math.min(6, candidates.length) && required.every(category => categories.has(category)) && required.length >= 4;
    return { repository: tree.repository, ref: tree.ref, findings, inspectionCoverage, inspectedFiles, limitsReached, inspectionSufficient };
  }
}
